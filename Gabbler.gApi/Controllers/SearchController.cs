...
namespace Gabbler.gApi.Controllers
{
    public class SearchController : ApiController
    {
        ...
        public async Task<IHttpActionResult> GetSearch([FromUri] string text)
        {
            try
            {
                var userSearchs = db.Users.Where(x => x.LastName.Contains(text) || x.FirstName.Contains(text) || x.Pseudo.Contains(text));
                var gabSearchs = db.Gabs.Where(x => x.Message.Contains(text));

                SearchListModel searchResult = new SearchListModel();
                searchResult.ListOfUser = userSearchs.ToList().ToUserBasicModel().ToList();
                searchResult.ListOfGab = gabSearchs.ToList().ToGabsList(0, 3);
